...
logFile = '../../Logs/Antikoerper/CSVExportJob.log'
...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) == 2:
            ...
            sql = "Select Voranmeldung.id,Nachname,Vorname,Telefon,Mailadresse,Station.Ort,Voranmeldung.Tag,Termine.Stunde,Termine.Slot from Voranmeldung LEFT JOIN Termine ON Termine.id=Voranmeldung.Termin_id LEFT JOIN Station ON Station.id=Termine.id_station where Voranmeldung.Tag Between '%s 00:00:00' and '%s 23:59:59' ORDER BY Termine.Stunde,Termine.Slot;" % (requestedDate.replace('-', '.'), requestedDate.replace('-', '.'))
        else:
        ...
        print(filename.replace('csv','xlsx').replace('../../Reports/Antikoerper/', ''))
        logger.info('Done')
